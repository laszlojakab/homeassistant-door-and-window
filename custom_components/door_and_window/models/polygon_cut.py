...
class baseVertex:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return math.isclose(other.x, self.x, abs_tol=0.001) and math.isclose(other.y, self.y, abs_tol=0.001)
...
def isVertexInPolygon(v, list):
    ...
    for i in range(len(list)):
        ...
        if floatEqual(maxY, minY):  # 同一水平线上
            if floatLarger(
                v.x, max(list[i % len(list)].x, list[j % len(list)].x)
            ):  # 点在线段右边 看成一个交点
                judgeIndex += 1
                continue
            elif floatLarger(
                min(list[i % len(list)].x, list[j % len(list)].x), v.x
            ):  # 点在线段左边 无交点
                continue
            else:  # 点在线段上
                return True
        # 做射线
        x = (list[i % len(list)].x - list[j % len(list)].x) / (
            list[i % len(list)].y - list[j % len(list)].y
...
def CutByVerticalLine(s1, s2, list):
    ...
    for i in range(len(list)):
        ...
        next = None
        if (
            (floatLarger(y, minY) and floatLarger(maxY, y))  # 交点在s1s2中
            # or (c2.y == y and x == s2.x)                # 线段交点在双方末端（在首端无视）
            # or (c1.y == y and x == s1.x)
            or (
                floatEqual(c2.x, x) and floatEqual(y, s1.y)
            )  # 交点在端点的时候，一条线段的首和另一条的尾才能有交点，上述注释做法在某种情况失效
            or (floatEqual(c1.x, x) and floatEqual(y, s2.y))
            or (
                floatEqual(y, minY)
                and (not floatEqual(c1.x, x))
                and (not floatEqual(c2.x, x))
            )  # 交点在s1s2一端
            or (
                floatEqual(y, maxY)
                and (not floatEqual(c1.x, x))
            ...
            while not (
                (isinstance(vertex, Vertex) and isinstance(vertex.next, Vertex))
                or (
                    isinstance(vertex, Intersection)
                    and isinstance(vertex.nextS, Vertex)
                )
            ):
                if isinstance(vertex, Vertex):
                    assert isinstance(vertex.next, Intersection)
                    if (
                        floatLarger(c2.x, c1.x) and floatLarger(vertex.next.x, inters.x)
                    ) or (
                        floatLarger(c1.x, c2.x) and floatLarger(inters.x, vertex.next.x)
                    ):  # c1c2的横坐标不可能相同，否则和s1s2重合
                        break
                    vertex = vertex.next
                else:
                    assert isinstance(vertex.nextS, Intersection)
                    if (
                        floatLarger(c2.x, c1.x)
                        and floatLarger(vertex.nextS.x, inters.x)
                    ) or (
                        floatLarger(c1.x, c2.x)
                        and floatLarger(inters.x, vertex.nextS.x)
                    ):
                        break
                    vertex = vertex.nextS
            if isinstance(vertex, Vertex):
...
def CutByLine(s1, s2, list):
    ...
    for i in range(len(list)):
        ...
        next = None
        if (
            (floatLarger(x, minX) and floatLarger(maxX, x))  # 交点在s1s2中
            # or (c2.y == y and x == s2.x)                # 线段交点在双方末端（在首端无视）
            # or (c1.y == y and x == s1.x)
            or (
                floatEqual(c2.y, y) and floatEqual(x, s1.x)
            )  # 交点在端点的时候，一条线段的首和另一条的尾才能有交点，上述注释做法在某种情况失效
            or (floatEqual(c1.y, y) and floatEqual(x, s2.x))
            or (
                floatEqual(x, minX)
                and (not floatEqual(c1.y, y))
                and (not floatEqual(c2.y, y))
            )  # 交点在s1s2一端
            or (
                floatEqual(x, maxX)
                and (not floatEqual(c1.y, y))
            ...
            while not (
                ...
                if isinstance(vertex, Vertex):
                    ...
                    if (
                        (
                            floatLarger(c2.x, c1.x)
                            and floatLarger(vertex.next.x, inters.x)
                        )
                        or (
                            floatLarger(c1.x, c2.x)
                            and floatLarger(inters.x, vertex.next.x)
                        )
                        or (
                            floatLarger(c1.y - c1.x * slope, c2.y - c2.x * slope)
                            and floatLarger(inters.y, vertex.next.y)
                        )
                        or (
                            floatLarger(c2.y - c2.x * slope, c1.y - c1.x * slope)
                            and floatLarger(vertex.next.y, inters.y)
                        )
                    ):  # 后两个是垂直情况只能用y坐标判断 y应该反错切转换回去
                        break
                    vertex = vertex.next
                else:
                    assert isinstance(vertex.nextS, Intersection)
                    if (
                        ...
                        or (
                            floatLarger(c2.y - c2.x * slope, c1.y - c1.x * slope)
                            and floatLarger(inters.y, vertex.nextS.y)
                        )
                        or (
                            floatLarger(c2.y - c2.x * slope, c1.y - c1.x * slope)
                            and floatLarger(vertex.nextS.y, inters.y)
                        )
                    ):
                        break
                    vertex = vertex.nextS
...
def encode(Str):
    ...
    for i in range(len(X)):
        if (not floatEqual(X[i], X[i - 1])) or (
            not floatEqual(Y[i], Y[i - 1])
        ):  # 去掉连续重复的点
...
def toClockwise(list):  # 转换为顺时针
    ...
    for i in range(len(list)):
        if list[i].x > maxX:
            ...
            mark_i = i
    v1 = Vertex(
        list[mark_i].x - list[mark_i - 1].x, list[mark_i].y - list[mark_i - 1].y
    )
    v2 = Vertex(
        list[(mark_i + 1) % len(list)].x - list[mark_i].x,
        list[(mark_i + 1) % len(list)].y - list[mark_i].y,
    )
    crossPr = v1.x * v2.y - v2.x * v1.y
    while floatEqual(crossPr, 0):
        mark_i += 1
        v2 = Vertex(
            list[(mark_i + 1) % len(list)].x - list[mark_i % len(list)].x,
